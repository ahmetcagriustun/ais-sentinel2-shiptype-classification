...
def s3_list_keys(bucket: str, prefix: str) -> List[str]:
    ...
    for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
        for obj in page.get("Contents", []):
            ...
            if "/." in k or ".ipynb_checkpoints" in k: continue
            base = os.path.basename(k).lower()
            if "tci" in base: continue  # skip TCI
            if not (k.lower().endswith(".tif") or k.lower().endswith(".tiff")): continue
            keys.append(k)
    return keys
...
CLASSES = ["Cargo","Tanker", "Fishing", "Passenger", "Sailing", "Pleasure"]
...
def _to_chw(arr: np.ndarray, expected_channels: int) -> np.ndarray:
    ...
    if expected_channels in axes:
        ...
        if axes[2] in (3,4,8,12): ch_axis = 2
        elif axes[0] in (3,4,8,12): ch_axis = 0
        else: ch_axis = int(np.argmin(axes))
    else:
...
class S2Dataset(Dataset):
    ...
    def __init__(self, samples: List[Tuple[str, int]], image_size: int,
                 norm_stats: Dict[str, np.ndarray], expected_channels: int,
                 train: bool = True):
    ...
    def _augment(self, x: np.ndarray) -> np.ndarray:
        if random.random() < 0.5: x = x[:, :, ::-1]  # H flip
        if random.random() < 0.5: x = x[:, ::-1, :]  # V flip
...
def save_confusion_matrix(y_true, y_pred, classes, out_png: str, normalize=False):
    ...
    plt.imshow(cm, interpolation="nearest"); plt.title("Confusion Matrix" + (" (norm)" if normalize else ""))
...
def compute_percentile_stats(
    ...
    for c in range(C):
        ...
        low = np.percentile(vals, 2.0); high = np.percentile(vals, 98.0)
        if high <= low: high = low + 1e-3
...
def make_loaders(X_train, X_val, X_test, image_size, norm_stats,
...
def train_one_fold(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y in dl_train:
            ...
            running_loss += loss.item() * x.size(0); n_train += x.size(0)
        train_loss = running_loss / max(1, n_train)

        # ---- Validate ----
        model.eval()
        val_loss = 0.0; n_val = 0; correct = 0
        with torch.no_grad():
            for x, y in dl_val:
                x = x.to(device, non_blocking=True); y = y.to(device, non_blocking=True)
                with torch.amp.autocast('cuda', enabled=mp_enabled):
                    logits = model(x); loss = criterion(logits, y)
                val_loss += loss.item() * x.size(0); n_val += x.size(0)
                pred = logits.argmax(dim=1); correct += (pred == y).sum().item()
        val_loss = val_loss / max(1, n_val); val_acc = correct / max(1, n_val)
...
def main():
    ...
    assert len(fold_test_indices) == K
    ...
    for r in range(K):
        ...
        cnt = Counter(train_labels); total = sum(cnt.values())
        class_weights = torch.tensor(
        ...
        norm_stats = compute_percentile_stats(
            X_train, image_size=tr["image_size"], per_class_limit=200,
        ...
        out = train_one_fold(
            ...
            epochs=tr["epochs"], lr=tr["learning_rate"], weight_decay=tr["weight_decay"],
        ...
        top2 = top_k_accuracy_score(y_true, np.array(out["y_prob_test"]), k=2, labels=list(range(num_classes)))
        ...
        with open(os.path.join(fold_dir, "metrics.json"), "w", encoding="utf-8") as f:
            json.dump({
                "fold": r, "test_accuracy": float(acc), "test_top2_accuracy": float(top2),
                "best_val_loss": float(out["best_val_loss"])
            }, f, indent=2)

        save_confusion_matrix(y_true, y_pred, CLASSES, os.path.join(fold_dir, "confusion_matrix.png"), normalize=False)
        save_confusion_matrix(y_true, y_pred, CLASSES, os.path.join(fold_dir, "confusion_matrix_norm.png"), normalize=True)
    ...
    cv_stats = {
        "acc_mean": float(df_cv["acc"].mean()), "acc_std": float(df_cv["acc"].std(ddof=0)),
        "macro_f1_mean": float(df_cv["macro_f1"].mean()), "macro_f1_std": float(df_cv["macro_f1"].std(ddof=0)),
        "weighted_f1_mean": float(df_cv["weighted_f1"].mean()), "weighted_f1_std": float(df_cv["weighted_f1"].std(ddof=0)),
        "top2_mean": float(df_cv["top2"].mean()), "top2_std": float(df_cv["top2"].std(ddof=0)),
        "folds": len(cv_results),
        "classes": CLASSES,
        "merge_info":"No class merges (6-class)",
    ...
    for i in range(agg_confusion.shape[0]):
        for j in range(agg_confusion.shape[1]):
            plt.text(j, i, str(agg_confusion[i, j]), ha="center", va="center",
                     color="white" if agg_confusion[i, j] > agg_confusion.max()/2 else "black")
    plt.tight_layout(); plt.savefig(os.path.join(out_root, "aggregate_confusion.png"), dpi=160); plt.close()
